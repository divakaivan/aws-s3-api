...
class Diff:
    ...
    def __str__(self) -> str:
        before_str = (
            "MISSING" if self.before is None else json.dumps(self.before, indent=2)
        )
        after_str = (
            "MISSING" if self.after is None else json.dumps(self.after, indent=2)
        )
        return (
            f"******** Diff at: {self.path}\nBEFORE: {before_str}\nAFTER: {after_str}"
        )
...
def main() -> None:
    ...
    if args.command == "generate":
        ...
        write_openapi_to_disk(
            openapi_schema=generated_openapi_schema, outfile_path=args.output_spec
        )
        ...
        if not schemas_match:
            print(
                "❌ Existing OpenAPI schema does not match generated schema. Differences:\n"
            )
...
def parse_args() -> Args:
    ...
    diff_parser = subparsers.add_parser(
        "generate-and-diff", help="Generate and compare OpenAPI schema"
    )
...
def get_diff_between_openapi_schemas(
    input_spec: Path, generated_spec: dict
) -> List[Diff]:
    """
...
def diff_dicts(
    dict_a: dict, dict_b: dict, path: str = ""
) -> Generator[Diff, None, None]:  # noqa: R701
    """
